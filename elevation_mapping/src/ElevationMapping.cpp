...
ElevationMapping::ElevationMapping(ros::NodeHandle &nodeHandle)
    ...
    Instrumentor::Get().BeginSession("Elevation Mapping");
...
void ElevationMapping::pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr &pointCloudMsg, bool publishPointCloud,
    ...
    if (!receivedFirstMatchingPointcloudAndPose_) {
        PROFILE_SCOPE("receivedFirstMatchingPointcloudAndPose_");
    ...
    if (!ignoreRobotMotionUpdates_) {
        PROFILE_SCOPE("Robot pose covariance");
    ...
    {
        PROFILE_SCOPE("Sensor processor of point cloud");
    ...
    // Update map location.
    {
        PROFILE_SCOPE("Update map location");
        updateMapLocation();
    }

    // Update map from motion prediction.
    if (!updatePrediction(lastPointCloudUpdateTime_)) {
        PROFILE_SCOPE("Update map from motion prediction");
    ...
    if (map_.enableContinuousCleanup_) {
        PROFILE_SCOPE("Continuous cleanup");
        ROS_DEBUG("Clearing elevation map before adding new point cloud.");
        map_.clear();
    }

    {
        PROFILE_SCOPE("Add point cloud to elevation map");
...
bool ElevationMapping::getRawSubmapServiceCallback(grid_map_msgs::GetGridMap::Request &request,
    ...
    grid_map::Position requestedSubmapPosition(request.position_x, request.position_y);
    grid_map::Length requestedSubmapLength(request.length_x, request.length_y);
    ROS_INFO("Elevation raw submap request: Position x=%f, y=%f, Length x=%f, y=%f.", requestedSubmapPosition.x(),
